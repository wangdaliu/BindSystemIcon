...
public class BindHandlerThread extends HandlerThread implements Handler.Callback {
    ...
    public boolean handleMessage(Message msg) {
        ...
        if ((localList != null) && (localList.size() > 0)) {
            ...
            if (BindView.bindContact && contactSet.contains(className)) {
                startActivity(BIND_CONTACT_ACTION, BindManager.getInstance().getContactClass());
                return true;
            }

            if (BindView.bindCall && callSet.contains(className)) {
                startActivity(BIND_DIAL_ACTION, BindManager.getInstance().getDialClass());
                return true;
            }

            if (BindView.bindSms && smsSet.contains(className)) {
                startActivity(BIND_SMS_ACTION, BindManager.getInstance().getSmsClass());
                return true;
            }
        }
        return false;
    ...
    private void startActivity(String action, Class c) {
    ...
    private void convertSet(Intent paramIntent, HashSet paramHashSet, boolean paramBoolean) {
        List<ResolveInfo> resolveInfos = mContext.getPackageManager().queryIntentActivities(paramIntent, PackageManager.MATCH_DEFAULT_ONLY);
